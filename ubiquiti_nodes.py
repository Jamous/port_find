class ubiquiti_edgeswitch_class:
    ...
    def find_mac_on_switch(self,node_details,ssh,regex):
        #Usses session_values['mac_large'] for mac address
        #Connect to node
        ...
        if net_connect:
            ...
            command_out = net_connect.send_command("show mac-addr-table  " + self.session_values['mac_large'])
            ...
            port = regex.regex_search('0/..',command_out)

            try:
                #Count number of MAC addresses on the interface.
                port_output = net_connect.send_command("show mac-addr-table interface " + port)
                num_on_interface = len(regex.regex_findall('standard_format',port_output))
            except:
                num_on_interface = 0
            ...
            if 0 < num_on_interface < self.session_values['number']:
                if not self.session_values['verbose']:
                    ...
                    standard_output += net_connect.send_command("show mac-addr-table " + self.session_values['mac_large'])
                    ...
                    logg_output += net_connect.send_command("show mac-addr-table " + self.session_values['mac_large'])
...
class ubiquiti_edgerouter_class:
    def __init__(self,session_values):
        self.session_values = session_values
    ...
    def find_arp_on_router(self,node_details,ssh,regex):
        #Usses session_values['mac_small'] for mac address
        net_connect = ssh.start_ssh_connection(node_details)
        
        #Actual commands happen here. This checks if net_connect failed
        if net_connect:
            #Get arp address from router
            command_out = net_connect.send_command("show arp | match " + self.session_values['address'])
            logg_output = command_out
            ...
            mac_address = regex.regex_search('standard_format',command_out)
